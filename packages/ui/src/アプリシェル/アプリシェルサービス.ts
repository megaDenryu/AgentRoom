...
export class アプリシェルサービス {
  ...
  private readonly _判定キュー: 問い判定キュー;
  ...
  private constructor(
    ...
    this._判定キュー = new 問い判定キュー(
      new 問いクライアント(),
      (件数) => this._部品.外殻.アクティビティバッジ件数を設定する(判定アクティビティ, 件数),
    );
  }
  ...
  札場タブを開く(): void {
    if (!this._部品.外殻.タブが存在するか(札場タブid)) {
      ...
      this._部品.外殻.タブ再読み込みボタンを追加する(札場タブid, () => 札場.更新する());
    }
  ...
  判定タブを開く(): void {
    if (!this._部品.外殻.タブが存在するか(判定タブid)) {
      this._部品.外殻.タブを追加する(判定タブid, "人間判定", this._判定キュー);
      this._部品.外殻.タブ再読み込みボタンを追加する(
        判定タブid,
        () => void this._判定キュー.更新する(),
      );
    }
    this._部品.外殻.タブを選択する(判定タブid);
  }
