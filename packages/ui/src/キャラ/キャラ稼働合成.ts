...
// 稼働状態はnull(未申告)を許容する。参加ルーム一覧は本合成の責務外(キャラ参加ルーム合成.ts参照)
// のためここでは空配列を既定値として持たせるだけに留める
export interface キャラ稼働合成 {
  readonly キャラ: キャラDTO;
  readonly 稼働状態: 稼働表明DTO["状態"] | null;
  readonly 参加ルーム一覧: readonly string[];
}

// 純粋関数(副作用なし)。presence合成専任であり、参加ルームの突き合わせは行わない。
// デスクトップのキャラタブ・モバイルのキャラ一覧ビューの双方が使う
