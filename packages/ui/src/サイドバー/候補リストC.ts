...
// 本来はSengenUI本体に追加すべき拡張だが、今回の変更対象はAgentRoomリポジトリに限定されるため、
// 既存プリミティブ(TextInputComponent等)と同じ構築パターンをこのファイル内だけに閉じ込めて用意する。
// document.createElementの直接呼び出しはこのクラスの内部実装に限定し、利用側(メンバー追加フォーム)
// からは宣言的メソッドのみで使う(参照: Fudaba packages/ui/src/カンバン/候補リストC.ts の同型実装)
