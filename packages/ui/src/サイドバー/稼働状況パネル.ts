...
export class 稼働状況パネル extends LV2HtmlComponentBase {
  ...
  private readonly _一覧領域 = new 稼働状況一覧領域(this._文言);
  ...
  async 更新する(): Promise<void> {
    try {
      ...
      this._一覧領域.全件を差し替える(
        一覧.map((表明) => new 稼働状況行View(表明, this._文言)),
      );
      this._状態表示.クリアする();
    } catch (エラー) {
      this._状態表示.エラーを表示する(
        エラー instanceof Error ? エラー.message : this._文言.稼働状況取得失敗,
      );
    }
  }
    ...
    return (
      div({ class: styles.稼働状況ルート }).childs([
          div({ class: styles.メンバーヘッダ }).child(
              span({ text: this._文言.稼働状況見出し, class: styles.メンバー見出し })),
          状態表示,
          一覧領域])
    );
  }
