...
import { 判定ナビ項目id, 下部ナビ項目一覧 } from "./ナビ項目一覧";
import { 埋め込み表示対象か } from "../埋め込みモード";
...
export class モバイルシェル部品 {
  ...
  static 作る(クライアント: Relayクライアント, Jimbo埋め込み中 = false): モバイルシェル部品 {
    // ルーム一覧ビュー・キャラ一覧ビューのシートと会話ビューの送信設定シートが同じ
    // ボトムシートを共有するため、他の部品より先に構築して渡す
    const ボトムシート起 = new ボトムシート();
    const ナビ項目 = 下部ナビ項目一覧.filter(({ id }) => 埋め込み表示対象か(id, Jimbo埋め込み中));
    const 下部ナビ起 = new 下部ナビ(ナビ項目);
