...
export class キャラ一覧ビューサービス {
  ...
  新規作成シートを開く(): void {
    const 内容 = new キャラシート内容(this._文言).配線する({
  ...
  編集シートを開く(キャラ: キャラDTO): void {
    const 内容 = new キャラシート内容(this._文言, キャラ).配線する({
      on保存: (入力) => void this._保存する(内容, 入力),
  ...
  private async _保存する(内容: キャラシート内容, 入力: キャラ入力): Promise<void> {
    ...
    try {
      ...
      内容.エラーを表示する(エラー instanceof Error ? エラー.message : this._文言.保存失敗);
    } finally {
      内容.保存中にする(false);
    }
  }
  ...
  private async _削除する(内容: キャラシート内容, 名前: string): Promise<void> {
    try {
      ...
      内容.エラーを表示する(エラー instanceof Error ? エラー.message : this._文言.削除失敗);
    }
  }
}
