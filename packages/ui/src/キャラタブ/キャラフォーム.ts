...
export class キャラフォーム extends LV2HtmlComponentBase implements I配線可能<Iキャラフォーム配線> {
  ...
  constructor() {
    super();
    this._名前入力 = textInput({ placeholder: "キャラ名", class: styles.入力 });
    this._種別セレクト = select({
    ...
    this._保存ボタン = button({ text: "作成", class: styles.保存ボタン }).onClick(() =>
      this._保存を発火する(),
    );
  ...
  クリアする(): void {
    ...
    this._保存ボタン.setTextContent("作成");
  }

  private _ルートを構築する(): DivC {
    return (
      div({ class: styles.フォーム側 }).childs([
          span({ text: "キャラを作成・編集", class: styles.見出し }),
          this._名前入力,
          this._種別セレクト,
          this._アイコン入力,
          this._プロンプト入力,
          this._行動パターン入力,
          this._保存ボタン,
          button({ text: "キャンセル", class: styles.キャンセルボタン }).onClick(() => {
            this.クリアする();
            this._配線.先.onキャンセル();
          })])
    );
